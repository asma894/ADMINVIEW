...
<div class="max-w-2xl">
    <div class="mb-6 flex items-center gap-4">
        <div class="h-12 w-12 overflow-hidden rounded-full border border-gray-200 dark:border-gray-600">
            <img src="{{ $editAdmin->avatar_url }}" alt="{{ $editAdmin->name }}" class="h-full w-full object-cover" />
        </div>
        <div>
            <h2 class="text-xl font-semibold text-gray-800 dark:text-white">Edit {{ $editAdmin->name }}</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400">Super Admin · Approved {{ $editAdmin->approved_at?->format('M d, Y') }}</p>
        </div>
    </div>

    <div class="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
        <form method="POST" action="{{ route('admin-management.update', $editAdmin) }}">
            @csrf @method('PATCH')
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="fname" value="{{ old('fname', $editAdmin->first_name) }}"
                ...
                <div>
                    ...
                    <input type="text" name="bio" value="{{ old('bio', $editAdmin->bio) }}"
                        class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white shadow-sm focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10" />
                </div>
            </div>
