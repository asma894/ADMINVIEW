...
<div class="max-w-2xl">
    <div class="mb-6 flex items-center gap-4">
        <div class="h-12 w-12 overflow-hidden rounded-full border border-gray-200 dark:border-gray-600">
            <img src="{{ $editUser->avatar_url }}" alt="{{ $editUser->name }}" class="h-full w-full object-cover" />
        </div>
        <div>
            <h2 class="text-xl font-semibold text-gray-800 dark:text-white">Edit {{ $editUser->name }}</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400">Joined {{ $editUser->created_at->format('M d, Y') }}</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('users.update', $editUser) }}">
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="fname" value="{{ old('fname', $editUser->first_name) }}"
                ...
                <div>
                    ...
                    <input type="text" name="lname" value="{{ old('lname', $editUser->last_name) }}"
                        class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 shadow-sm focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10 dark:border-gray-600 dark:bg-gray-900 dark:text-white" />
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone', $editUser->phone) }}" placeholder="+1 555 000 000"
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Bio / Role</label>
                    <input type="text" name="bio" value="{{ old('bio', $editUser->bio) }}" placeholder="e.g. Editor"
                        class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white shadow-sm focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10" />
                </div>
            </div>
            ...
            <div class="mt-5 flex items-center gap-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/40 px-4 py-3">
                ...
                <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium
                    ...
                    <span class="h-1.5 w-1.5 rounded-full {{ $editUser->status === 'active' ? 'bg-green-500' : 'bg-red-500' }}"></span>
                    {{ ucfirst($editUser->status) }}
                </span>
                <form method="POST" action="{{ route('users.toggle-status', $editUser) }}" class="ml-auto">
                    @csrf @method('PATCH')
                    <button type="submit"
